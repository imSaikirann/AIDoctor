...
async function getOrCreateEmergencyPatient(email) {
    ...
    if (existing) {
        if (existing.role !== "PATIENT") {
            throw new EmergencyBookingError(400, "This email belongs to a non-patient account. Please use a different email.");
        }
        return existing;
    }
...
router.post("/emergency", async (req, res) => {
    try {
        ...
        if (!selectedDoctor || !selectedDoctor.verified) {
            throw new EmergencyBookingError(404, "Selected doctor is not available for emergency booking.");
        }
        const patient = await getOrCreateEmergencyPatient(email);
        const now = new Date();
        const slot = `Emergency ${now.toISOString()}`;
        const meetingUrl = buildEmergencyMeetingUrl(selectedDoctor.calLink, selectedDoctor.id);
